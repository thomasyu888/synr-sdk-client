...
#'
#' @field results  list( \link{UserGroup} ) [optional]
#'
...
PaginatedResultsOfUserGroup <- R6::R6Class(
  ...
  public = list(
    ...
    fromJSON = function(PaginatedResultsOfUserGroupJson) {
      PaginatedResultsOfUserGroupObject <- jsonlite::fromJSON(PaginatedResultsOfUserGroupJson)
      if (!is.null(PaginatedResultsOfUserGroupObject$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(PaginatedResultsOfUserGroupObject$`results`, "array[UserGroup]", loadNamespace("synclient"))
      }
      if (!is.null(PaginatedResultsOfUserGroupObject$`totalNumberOfResults`)) {
        self$`totalNumberOfResults` <- PaginatedResultsOfUserGroupObject$`totalNumberOfResults`
      }
      self
    },
...
',
    ...
    fromJSONString = function(PaginatedResultsOfUserGroupJson) {
      PaginatedResultsOfUserGroupObject <- jsonlite::fromJSON(PaginatedResultsOfUserGroupJson)
      self$`results` <- ApiClient$new()$deserializeObj(PaginatedResultsOfUserGroupObject$`results`, "array[UserGroup]", loadNamespace("synclient"))
      self$`totalNumberOfResults` <- PaginatedResultsOfUserGroupObject$`totalNumberOfResults`
      self
