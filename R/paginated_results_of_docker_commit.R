...
#' @field results  list( \link{DockerCommit} ) [optional]
#'
...
PaginatedResultsOfDockerCommit <- R6::R6Class(
  ...
  public = list(
    ...
    fromJSON = function(PaginatedResultsOfDockerCommitJson) {
      PaginatedResultsOfDockerCommitObject <- jsonlite::fromJSON(PaginatedResultsOfDockerCommitJson)
      if (!is.null(PaginatedResultsOfDockerCommitObject$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(PaginatedResultsOfDockerCommitObject$`results`, "array[DockerCommit]", loadNamespace("synclient"))
      }
      if (!is.null(PaginatedResultsOfDockerCommitObject$`totalNumberOfResults`)) {
        self$`totalNumberOfResults` <- PaginatedResultsOfDockerCommitObject$`totalNumberOfResults`
      }
    },
...
',
    ...
    fromJSONString = function(PaginatedResultsOfDockerCommitJson) {
      PaginatedResultsOfDockerCommitObject <- jsonlite::fromJSON(PaginatedResultsOfDockerCommitJson)
      self$`results` <- ApiClient$new()$deserializeObj(PaginatedResultsOfDockerCommitObject$`results`, "array[DockerCommit]", loadNamespace("synclient"))
      self$`totalNumberOfResults` <- PaginatedResultsOfDockerCommitObject$`totalNumberOfResults`
      self
